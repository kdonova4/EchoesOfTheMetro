...
public class EventControllerTest
{
    ...
    void setup() {
        ...
        event = new Event(1, "You found Artyom's home", EventType.STANDARD, 5, 0, 0, null, null, null, null);
    ...
    void shouldFindByIdShouldReturn200() throws Exception {
        ...
        String eventJson = jsonMapper.writeValueAsString(eventResponse);

        var request = get("/api/events/1")
    ...
    @Test
    void generateEventShouldReturn404() throws Exception {
        when(eventService.generateEvent(anyInt(), anyInt())).thenThrow(NoSuchElementException.class);

        var request = get("/api/events/event/1/1")
                .header("Authorization", "Bearer " + token);
    ...
    @Test
    void generateEventShouldReturn200() throws Exception {
        when(eventService.generateEvent(anyInt(), anyInt())).thenReturn(event);

        String eventJson = jsonMapper.writeValueAsString(EventMapper.toResponse(event));
